...
final class RouterOsSshClient implements MikrotikClientInterface
{
    ...
    public function command(string $command, array $arguments = []): array
    {
        ...
        $cli = $this->buildCommand($command, $arguments);
        $output = $this->ssh->exec($cli);
        if ($output === false) {
            throw new RuntimeException('MikroTik SSH command failed.');
        }

        $this->assertNoRouterOsError((string)$output);
        return $this->parseRows((string)$output);
    }
    ...
    private function buildCommand(string $command, array $arguments): string
    {
        ...
        if (in_array($action, ['set', 'disable', 'enable'], true)) {
            ...
            $commandLine = $base . ' [find where ' . $selector . ']';
            foreach ($arguments as $key => $value) {
                $key = (string)$key;
                if ($value === null || $key === '.id' || str_starts_with($key, '?')) continue;
                $commandLine .= ' ' . $key . '=' . $this->quote((string)$value);
            }
            return $commandLine;
        }
    ...
    private function assertNoRouterOsError(string $output): void
    {
        foreach (preg_split('/\R/', $output) ?: [] as $line) {
            $line = trim($line);
            if ($line === '') continue;
            if (preg_match('/^(failure|input does not match|expected end of command|bad command|syntax error)/i', $line)) {

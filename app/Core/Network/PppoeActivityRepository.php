...
final class PppoeActivityRepository
{
 ...
 public function markRouterSessionsOffline(int $tenantId,int $routerId):void{$s=$this->pdo->prepare("UPDATE pppoe_activity_state SET online=FALSE,stale=FALSE,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=:t AND router_id=:r");$s->execute([':t'=>$tenantId,':r'=>$routerId]);}
 ...
 public function find(int $tenantId,int $routerId,string $username):?array{$s=$this->pdo->prepare("SELECT username,online,active_ip,uptime_seconds,rx_bytes,tx_bytes,rx_rate_bps,tx_rate_bps,last_seen_at,updated_at,stale FROM pppoe_activity_state WHERE tenant_id=:t AND router_id=:r AND username=:u LIMIT 1");$s->execute([':t'=>$tenantId,':r'=>$routerId,':u'=>$username]);$row=$s->fetch(PDO::FETCH_ASSOC);return $row!==false?$row:null;}
}

...
final class PppoeUsageAggregator
{
 ...
 public function purgeOlderThanMonths(int $months=6):int{$months=max(6,$months);$cutoff=(new DateTimeImmutable('first day of this month'))->modify("-{$months} months");$q=$this->pdo->prepare('DELETE FROM pppoe_usage_hourly WHERE bucket_start < :cutoff');$q->execute([':cutoff'=>$cutoff->format('Y-m-d H:i:s')]);return $q->rowCount();}
}

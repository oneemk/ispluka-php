...
final class MikrotikRouterController
{
    ...
    private function input(Request $request,bool $creating):array{$verify=$request->input('verify_ssl',false);$verifySsl=filter_var($verify,FILTER_VALIDATE_BOOLEAN);return ['name'=>$request->input('name'),'code'=>$request->input('code'),'host'=>$request->input('host'),'connection_method'=>$request->input('connection_method','api'),'api_port'=>$request->input('api_port',8728),'api_ssl_port'=>$request->input('api_ssl_port'),'ssh_port'=>$request->input('ssh_port',22),'username'=>$request->input('username'),'password'=>$request->input('password'),'verify_ssl'=>$verifySsl];}

...
final class HotspotApiController
{
    ...
    {
        return $this->json(fn () => $this->hotspot->profiles($this->tenantId()));
    }
    ...
    public function createProfile(Request $request): Response
    {
        try {
            $id = $this->crud->createProfile($this->tenantId(), [
                ...
                'session_limit_seconds' => $request->input('session_limit_seconds'),
                'shared_users' => $request->input('shared_users', 1),
            ]);
    ...
    {
        return $this->json(fn () => $this->hotspot->users($this->tenantId()));
    }
    ...
    public function disconnect(Request $request): Response
    {
        try {
            ...
            $this->actions->disconnect($this->tenantId(), $sessionId);
    ...
    public function routerTime(Request $request): Response
    {
        try {
            ...
            return Response::json(['data' => $this->actions->syncRouterTime($this->tenantId(), $routerId)]);
        } catch (Throwable $e) {
            return $this->error($e, 422);
        }
    }
    ...
    public function activeUsers(Request $request): Response
    {
        try {
            ...
            return Response::json(['data' => $this->actions->activeUsers($this->tenantId(), $routerId)]);
        } catch (Throwable $e) {
            return $this->error($e, 422);
        }
    }
    ...
    {
        return $this->toggleUser($request, false);
    }

    public function enableUser(Request $request): Response
    {
        return $this->toggleUser($request, true);
    }
    ...
    private function toggleUser(Request $request, bool $enable): Response
    {
        try {
            ...
            if ($routerId < 1 || $username === '') {
                throw new InvalidArgumentException('Router ID and Hotspot username are required.');
            }
            if ($enable) {
                $this->actions->enableRouterUser($this->tenantId(), $routerId, $username);
            } else {
                $this->actions->disableRouterUser($this->tenantId(), $routerId, $username);
            }
    ...
    private function error(Throwable $e, int $status = 400): Response
    {
        $message = $e instanceof InvalidArgumentException || $e instanceof \RuntimeException
            ? $e->getMessage()
            : 'Unable to process Hotspot request.';

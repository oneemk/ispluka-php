...
{
    public function __construct(private readonly PDO $pdo, private readonly Database $db, private readonly AuthManager $auth, private readonly SecretBox $secrets, private readonly MikroTikHotspotGateway $gateway) {}
    ...
    public function activate(Request $r): \Ispluka\Core\Http\Response { $tenant=$this->tenant();$id=(int)$r->input('user_id',0);$result=(new HotspotValidityService($this->db))->activate($tenant,$id);$this->log($tenant,null,$id,'user.activate','success');return ApiResponse::success($result); }

...
final class LoginController
{
    ...
    public function show(Request $request):Response{
        if($this->auth->check())return Response::redirect('/');$token=htmlspecialchars($this->csrf->token(),ENT_QUOTES,'UTF-8');$ok=isset($_GET['signup'])&&$_GET['signup']==='success'?'<div class="card" style="border-color:#86efac;background:#f0fdf4;color:#166534">Account created. Sign in to start your trial.</div>':'';
    ...
    public function login(Request $request):Response{
        if(!$this->csrf->validate((string)$request->input('_csrf','')))return Response::text('Invalid CSRF token.',419);if(!$this->auth->attempt((string)$request->input('login',''),(string)$request->input('password','')))return Response::text('Invalid credentials or account temporarily locked.',422);return Response::redirect('/');
    }
    public function logout(Request $request):Response{if(!$this->csrf->validate((string)$request->input('_csrf','')))return Response::text('Invalid CSRF token.',419);$this->auth->logout();return Response::redirect('/login');}
}

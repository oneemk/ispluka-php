...
final class RouterService
{
    ...
    public function create(int $tenantId, array $data): int
    {
        ...
        $apiPort = (int)($data['api_port'] ?? 8728);
        ...
        return $this->routers->create($tenantId, [
            'name'=>$name, 'code'=>$code, 'host'=>$host, 'api_port'=>$apiPort,
    ...
    {
        $this->provisionUser($tenantId, $routerId, '/ppp/secret/print', '/ppp/secret/add', '/ppp/secret/set', $username, [
    ...
    {
        $this->provisionUser($tenantId, $routerId, '/ip/hotspot/user/print', '/ip/hotspot/user/add', '/ip/hotspot/user/set', $username, [
    ...
    private function provisionUser(int $tenantId, int $routerId, string $findCommand, string $addCommand, string $setCommand, string $username, array $data): void
    {
        ...
        $client = $this->client($router);
        try {
            $rows = $client->command($findCommand, ['?name'=>$username]);
            if (!empty($rows[0]['=.id'])) {
                $client->command($setCommand, array_merge(['numbers'=>$rows[0]['=.id']], $data));
            } else {
                $client->command($addCommand, $data);
            }
            $this->routers->markConnection($tenantId, $routerId, true);
        } catch (\Throwable $e) {
            $this->routers->markConnection($tenantId, $routerId, false, $e->getMessage());
    ...
    private function toggleUser(int $tenantId, int $routerId, string $findCommand, string $actionCommand, string $username): void
    {
        ...
        try {
            $rows = $client->command($findCommand, ['?name'=>$username]);
            if (empty($rows[0]['=.id'])) throw new RuntimeException('Service account not found.');
            $client->command($actionCommand, ['numbers'=>$rows[0]['=.id']]);
            $this->routers->markConnection($tenantId, $routerId, true);
        } catch (\Throwable $e) {
            $this->routers->markConnection($tenantId, $routerId, false, $e->getMessage());

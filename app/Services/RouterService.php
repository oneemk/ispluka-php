...
final class RouterService
{
    ...
    {
        $rows=$this->routers->list($tenantId);
        foreach($rows as &$row){$result=$this->testConnection($tenantId,(int)$row['id']);$row['status']=$result['ok']?'online':'offline';$row['last_error']=$result['ok']?null:$result['error'];}
    ...
    public function create(int $tenantId,array $data): int
    {
        ...
        $this->validatePorts($apiPort,$data['api_ssl_port']??null);
    ...
    public function update(int $tenantId,int $routerId,array $data): void
    {
        ...
        $this->validatePorts($apiPort,$ssl);$sslPort=($ssl!==null&&$ssl!=='')?(int)$ssl:null;
        $payload=['name'=>$name,'code'=>$code,'host'=>$host,'api_port'=>$apiPort,'api_ssl_port'=>$sslPort,'username'=>$username,'verify_ssl'=>(bool)($data['verify_ssl']??false)];
        $password=(string)($data['password']??'');if($password!=='')$payload['encrypted_password']=$this->secrets->encrypt($password);
    ...
    {
        $router=$this->routers->find($tenantId,$routerId);if($router===null) throw new RuntimeException('Router not found.');
        try{$this->client->connect($this->routerConfig($router));$identity=$this->client->command('/system/identity/print');$this->routers->markConnection($tenantId,$routerId,true);return ['ok'=>true,'identity'=>$identity[0]['name']??null];}
        catch(\Throwable $e){$message=trim($e->getMessage())!==''?$e->getMessage():'Unknown MikroTik connection error.';$this->routers->markConnection($tenantId,$routerId,false,$message);return ['ok'=>false,'error'=>$message];}

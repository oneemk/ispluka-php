...
final class RouterService
{
    ...
    public function update(int $tenantId,int $routerId,array $data): void
    {
        ...
        $ports=$this->validatePorts($method,$data['api_port']??null,$data['api_ssl_port']??null,$data['ssh_port']??null);
        $payload=['name'=>$name,'code'=>$code,'host'=>$host,'connection_method'=>$method,'api_port'=>$ports['api_port'],'api_ssl_port'=>$ports['api_ssl_port'],'ssh_port'=>$ports['ssh_port'],'username'=>$username,'verify_ssl'=>(bool)($data['verify_ssl']??false),'status'=>'unknown','last_error'=>null];
    ...
    public function testConnection(int $tenantId,int $routerId): array
    {
        ...
        try{$this->client->connect($this->routerConfig($router));$identity=$this->client->command('/system/identity/print');$this->routers->markConnection($actualTenantId,$routerId,true);return ['ok'=>true,'status'=>'online','connection_method'=>$method,'identity'=>$identity[0]['name']??null];}
        catch(\Throwable $e){$message=trim($e->getMessage())!==''?$e->getMessage():'Unknown MikroTik connection error.';$this->routers->markConnection($actualTenantId,$routerId,false,$message);return ['ok'=>false,'status'=>'offline','connection_method'=>$method,'error'=>$message];}

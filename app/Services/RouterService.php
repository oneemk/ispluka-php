...
final class RouterService
{
    ...
    public function update(int $tenantId,int $routerId,array $data): void
    {
        ...
        $payload=['name'=>$name,'code'=>$code,'host'=>$host,'api_port'=>$apiPort,'api_ssl_port'=>$sslPort,'username'=>$username,'verify_ssl'=>(bool)($data['verify_ssl']??false),'status'=>'offline','last_error'=>null];
    ...
    {
        $router=$this->routers->find($tenantId,$routerId);if($router===null) throw new RuntimeException('Router not found.');
        try{$this->client->connect($this->routerConfig($router));$identity=$this->client->command('/system/identity/print');$this->routers->markConnection($tenantId,$routerId,true);return ['ok'=>true,'status'=>'online','identity'=>$identity[0]['name']??null];}
        catch(\Throwable $e){$message=trim($e->getMessage())!==''?$e->getMessage():'Unknown MikroTik connection error.';$this->routers->markConnection($tenantId,$routerId,false,$message);return ['ok'=>false,'status'=>'offline','error'=>$message];}

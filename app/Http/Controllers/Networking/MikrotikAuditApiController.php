...
final class MikrotikAuditApiController
{
    ...
    {
        $tenantId=(int)$this->auth->userId();
        if($tenantId<1)return Response::json(['error'=>['message'=>'Authentication required.']],401);
        $status=(string)($request->input('status')??'');$severity=(string)($request->input('severity')??'');$router=(int)($request->input('router_id')??0);

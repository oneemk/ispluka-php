...
final class RouterRepository
{
    ...
    public function findAny(int $routerId): ?array { $stmt=$this->database->pdo()->prepare('SELECT * FROM routers WHERE id = :id LIMIT 1');$stmt->execute([':id'=>$routerId]);$row=$stmt->fetch(PDO::FETCH_ASSOC);return $row?:null; }
    ...
    {
        if($success){$stmt=$this->database->pdo()->prepare('UPDATE routers SET status=:status,last_seen_at=CURRENT_TIMESTAMP,last_error=NULL,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=:tenant_id AND id=:id');$stmt->execute([':status'=>'online',':tenant_id'=>$tenantId,':id'=>$routerId]);return;}
        $stmt=$this->database->pdo()->prepare('UPDATE routers SET status=:status,last_error=:error,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=:tenant_id AND id=:id');$stmt->execute([':status'=>'offline',':error'=>mb_substr((string)$error,0,1000),':tenant_id'=>$tenantId,':id'=>$routerId]);
    }
}

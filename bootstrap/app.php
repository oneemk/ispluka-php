...
use Ispluka\Controllers\NetworkModuleController;
...
use Ispluka\Core\Hotspot\HotspotOperationalService;
...
$customerNetworkingController = new CustomerNetworkingController($auth, $csrf);
...
$networkModuleController = new NetworkModuleController($auth, $csrf);
...
$hotspotCrud = new HotspotCrudService($database->pdo(), $hotspotValidity, $secretBox);
...
$hotspotOperations = new HotspotOperationalService($database->pdo(), $hotspotGateway, $hotspotAudit);
$hotspotController = new HotspotApiController($hotspotRepository, $hotspotCrud, $hotspotActions, $hotspotAudit, $hotspotOperations, $auth);
...
if (is_file($webRoutes)) {
    ...
    if (is_callable($registerRoutes)) {
        $registerRoutes(
            ...
            $hotspotController,
            $networkModuleController,
            $customerNetworkingController,
        );
    }
}
